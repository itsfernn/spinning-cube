...
int pointInTriangle(struct vec2 p, struct vec2 a, struct vec2 b,
                    struct vec2 c) {
...
void draw(struct vec2 faces[3][4], char chars[], struct winsize *w) {
  ...
  for (int i = y; i < size_y; i++) {
    for (int j = x; j < size_x; j++) {
      struct vec2 P = {((double)j + 0.5) / w->ws_col * aspectRatio.x,
...
void projectQuadToTerminal(struct vec3 *quad, struct vec2 *projected_quad,
...
int getVisibleFaces(struct vec3 rotation) {
  ...
  for (int i = 0; i < 6; i++) {
    double dp =
        normals[i].x * view.x + normals[i].y * view.y + normals[i].z * view.z;
    if (dp > 0)
      mask |= (1 << i);
  }
  return mask;
...
void handleEdgeCollision(struct vec2 faces[3][4], struct cube *cube) {
  ...
  if (bounds.min.x >= 0 && bounds.max.x <= aspectRatio.x && bounds.min.y >= 0 &&
      bounds.max.y <= aspectRatio.y) {
    return; // no collision
  }
...
int handleKeyboardInput(struct cube *cube) {
  ...
  if (c == '-') {
    cube->size -= 0.05;
    if (cube->size < 0.05)
      cube->size = 0.05; // minimum size
  } else if (c == '+') {
    cube->size += 0.05;
    if (cube->size > 1.0)
      cube->size = 1.0; // maximum size
  }
